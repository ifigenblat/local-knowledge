      ...
      `}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            {/* Admin Section */}
            {(isAdmin(user) || hasPermission(user, 'roles.view') || hasPermission(user, 'users.view')) && (
              <div className="pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                {hasPermission(user, 'users.view') && (
                  <Link
                    to="/users"
                    onClick={handleLinkClick}
                    className={`flex items-center px-3 py-2 text-sm font-medium rounded-lg border transition-colors ${getActiveClass('/users')}`}
                  >
                    <Users className="w-5 h-5 mr-3" />
                    Users
                  </Link>
                )}
                {(isAdmin(user) || hasPermission(user, 'roles.view')) && (
                  <Link
                    to="/roles"
                    onClick={handleLinkClick}
                    className={`flex items-center px-3 py-2 text-sm font-medium rounded-lg border transition-colors ${getActiveClass('/roles')}`}
                  >
